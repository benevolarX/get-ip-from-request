...
export const ipv4 = `((?:(?:${v4seg})\\.){3}(?:${v4seg}))`
const hexa = '([0-9A-Fa-f]{1,4})'
export const ipv6 = `((((${hexa}:){7}(${hexa}|:))|((${hexa}:){6}(:${hexa}|(${ipv4})|:))|((${hexa}:){5}(((:${hexa}){1,2})|:(${ipv4})|:))|((${hexa}:){4}(((:${hexa}){1,3})|((:${hexa})?:(${ipv4}))|:))|((${hexa}:){3}(((:${hexa}){1,4})|((:${hexa}){0,2}:(${ipv4}))|:))|((${hexa}:){2}(((:${hexa}){1,5})|((:${hexa}){0,3}:(${ipv4}))|:))|((${hexa}:){1}(((:${hexa}){1,6})|((:${hexa}){0,4}:(${ipv4}))|:))|(:(((:${hexa}){1,7})|((:${hexa}){0,5}:(${ipv4}))|:)))(%.+)?)`;
...
export const isIp = (v) => regexip.test(v)
...
export function getIpFromRequest(req, fields = []) {
  if (req?.headers) {
    ...
    for (const index of checkHeaders) {
